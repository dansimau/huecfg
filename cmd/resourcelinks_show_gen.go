...
type resourcelinksShowCmd struct {
	Arguments struct {
		ID string
	} `positional-args:"true" required:"true" positional-arg-name:"resourcelink-ID"`
}

func (c *resourcelinksShowCmd) showCmdPostProcessFuncExists() bool {
	return reflect.ValueOf(c).MethodByName("PostProcessShowCmd").Kind() != reflect.Invalid
}

func (c *resourcelinksShowCmd) showCmdPostProcessFuncCall(bytes []byte) ([]byte, error) {
...
func (c *resourcelinksShowCmd) Execute(args []string) error {
	bridge := cmd.getHue()

	resp, err := bridge.GetResourceLink(c.Arguments.ID)
	if err != nil {
		return err
	}
